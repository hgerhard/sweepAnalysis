...
dataDir = '~/Experiments/3DThresh/Data/Pilot_ThreeSFs/2Hz/Proj_Exp_TEXT_HCN_128_Avg/';
...
for freqNum = 2%:length(pdData(1,1).avFreqs)
    ...
    for chanNum = 1:length(channels)
        ...
        figNum = plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,condNum), [figNum,subplotLoc(chanNum,:)]);
        ...
        condNum = 4;
        plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(1,:,condNum-3), [figNum,subplotLoc(chanNum,:)]);
...
for freqNum = 2%:length(pdData(1,1).avFreqs)
    for chanNum = 4 % 75 only
        ...
        plotPolarBins( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, [0.3 0.3 0.3], figNum ); % allColors(1,:,condNum-3)
        %print('-dpsc', '-append', sprintf('SFPilot2Hz_%s.ps',pdData(condNum,chanNum).avFreqs{freqNum}))
        %close;
        ...
        condNum = 5;
        plotPolarBins( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, [0.3 0.3 0.3], figNum );
        %print('-dpsc', '-append', sprintf('SFPilot2Hz_%s.ps',pdData(condNum,chanNum).avFreqs{freqNum}))
        %close;
