...
%%   - Classic sweep plots of power as a fxn of bin value, with BOOTSTRAPPED error bars
for freqNum = 2%:length(pdData(1,1).avFreqs)
    figNum = 1;
    for chanNum = 1:length(channels)
        
        condNum = 1;
        figNum = plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,3), [figNum,subplotLoc(chanNum,:)], 'Boot', ...
            [], bootData(condNum,chanNum,freqNum).resampledVectAmplitudes );
        
        condNum = 2;
        plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,1), [figNum,subplotLoc(chanNum,:)], 'Boot', ...
            [], bootData(condNum,chanNum,freqNum).resampledVectAmplitudes );
...
%%   - Classic sweep plots of power as a fxn of bin value, with error STDERROR error bars
for freqNum = 2%:length(pdData(1,1).avFreqs)
    figNum = 1;
    for chanNum = 1:length(channels)
        ...
        tmp = pdData(condNum,chanNum).dataMatr; %### put this inside plotSweep?? or sep. fxn. to grab appropriate data?
        stdErrVals = tmp(:,[find(strcmp('iTrial',colHdr)),find(strcmp('iBin',colHdr)),find(strcmp('iFr',colHdr)),find(strcmp('StdErr',colHdr))]); % ###
        stdErrVals = stdErrVals(stdErrVals(:,1)==0,:); % keep only 0th subject
        stdErrVals = stdErrVals(stdErrVals(:,3)==freqNum,:); % keep only desired freqNum
        stdErrVals = stdErrVals(stdErrVals(:,2)>0,:); % remove 0th bin
        stdErrVals = stdErrVals(:,end)'; % keep only the std errors
        
        figNum = plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,3), [figNum,subplotLoc(chanNum,:)], 'PDStdErr', ...
            [], stdErrVals );
        ...
        plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,1), [figNum,subplotLoc(chanNum,:)], 'PDStdErr', ...
            [], stdErrVals );
...
%%   - Classic sweep plots of power as a fxn of bin value, with SEM error bars
for freqNum = 1:length(pdData(1,1).avFreqs)
    ...
    for chanNum = 1:length(channels)
        ...
        condNum = 1;
        figNum = plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,3), [figNum,subplotLoc(chanNum,:)], 'SEM');
        
        condNum = 2;
        plotSweep( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,1), [figNum,subplotLoc(chanNum,:)], 'SEM');
    ...
    print('-dpsc', '-append', sprintf('BabyDisp2Hz_%s.ps',pdData(condNum,chanNum).avFreqs{freqNum}))
    close;
end
...
for freqNum = 1:length(pdData(1,1).avFreqs)
    for chanNum = 4 % 75 only
        ...
        figNum = plotPolarBins( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,3) );
        condNum = 2;
        plotPolarBins( pdData(condNum,chanNum).dataMatr, ...
            colHdr, pdData(condNum,chanNum).binLevels, freqNum, allColors(end,:,1), figNum );
        print('-dpsc', '-append', sprintf('BabyDisp2Hz_%s.ps',pdData(condNum,chanNum).avFreqs{freqNum}))
        close;
